...
char **v;
{
	...
	for (readvol=0;;)
	{
		if ((tape=open("/dev/rmt/c1t7d0hn",O_RDONLY))<0)
		{
		...
		if ((tape=open("/dev/rmt/c1t7d0hn",O_RDONLY))<0)		/* open to read next file			*/
		{
			perror("error on open");
			exit(2);
		}
		...
		*p++ = (char)0;							/* put in a null to make a double string	*/
		...
		if ((tape=open("/dev/rmt/c1t7d0hn",O_RDONLY))<0)		/* open to read eof trailers			*/
		{
			perror("error on open");
			exit(2);
		}
...
rew()
{
	printf("rewinding...\n");
	close(tape);
	tape=open("/dev/rmt/c1t7d0h",O_RDONLY);
	close(tape);
	exit(2);
}
lower(p)
