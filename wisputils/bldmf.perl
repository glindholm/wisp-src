...
#
	...
	$ccblcmd = "ccbl";
	$output = "makefile";
	$wispexecdef = "/usr/bin/wispbin/wisp";
	...
	while ($_ = $ARGV[0], /^-/)
	{
		...
		if (/^-a/)
		{
			...
			$ccblcmd  = "cob";
			$langflag = "-VAIX";
			$ccblswitch = $ccblswitch . "-C warning=2";
		}
		/^-cc/ && ($compcob = 1);
		/^-q/ && ($quietmode = 1);
		if (/^-o(.*)/)
		...
		if ($_ eq "-h")
		{
			...
			print "\t-q\t\t\tQuiet mode\n";
			print "\t-a\t\t\tBuild makefile for AIX COBOL build\n";
	...
	print MK "WISPFLAGS = $wispswitch\n";
	print MK "CCBL      = $ccblcmd\n";
	print MK "CCBLFLAGS = $ccblswitch\n";
	print MK "RUNDIR    = $dest\n\n";
	print MK "LANGUAGE  = $langflag\n\n";
	...
	while ($#wcbs >= $[) {			# cycle thru
		...
		$file = "\$(RUNDIR)/" . $file;
		print MK "\t$file";		# build a complete object ../run/xxxx
		$runnames{$save} = $file;
...
#		print MK "$filecob: \$(WISP) $filewcb";
		print MK "$runnames{$filewcb}: $filewcb";
		if ($#deps >= $[) { print MK "\\\n"; } else { print MK "\n"; }
		while ($#deps >= $[) 
		{
			$thisdep = pop(@deps);
		...
		print MK "\t\$(WISP) \$(WISPFLAGS) \$(LANGUAGE) $filewcb\n";
		if ($logerr) 
		{
			if ($coboltype eq "ACU")
			{
				print MK "\t\$(CCBL) \$(CCBLFLAGS) $alignopt -o $runnames{$filewcb} $filecob >>$fileerr 2>&1 "; 
			}
			else 
			{
				if ($coboltype eq "AIX")
				{
				  print MK "\t\$(CCBL) \$(CCBLFLAGS) $filecob >>$fileerr 2>&1 "; 
				}
			}
		}
		else 
		{ 
			if ($coboltype eq "ACU")
			{
				print MK "\t\$(CCBL) \$(CCBLFLAGS) $alignopt -o $runnames{$filewcb} $filecob "; 
			}
			else
			{
				if ($coboltype eq "AIX")
				{
				  print MK "\t\$(CCBL) \$(CCBLFLAGS) $filecob";
				}
			}
                }
