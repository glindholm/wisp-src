...
EDLOAD() {init_screen(); ede_synch = TRUE;}						/* Enable the EDE environment.		*/
...
MENULOAD(md, t, o, r, c, w) struct video_menu *md; long *t, *o, *r, *c, *w;		/* Get the menu data.			*/
{
	vmenuinit(md, (int)*t, (int)*o, (int)*r, (int)*c, (int)*w);			/* Do the call.				*/
}

MENUITEM(md, t, v, p) struct video_menu *md; unsigned char *t; long *v, *p;		/* Get the item data.			*/
{
...
MENUGO(md, v) struct video_menu *md; long *v;						/* Run the menu.			*/
{
	*v = (long)vmenugo(md);								/* Call the menu manager.		*/
}

MENUCONT(md, v) struct video_menu *md; long *v;						/* Continue the menu.			*/
{
	*v = (long)vmenucont(md);							/* Call the menu manager.		*/
}
...
MENUKILL(md, how) struct video_menu *md; long *how;					/* Erase menus.				*/
{
...
MENUMODE(mode) long *mode;								/* Menu mode.				*/
{
	vmenumode((int)*mode);								/* Select the menu mode.		*/
}

VIDMODE(mode) long *mode;								/* Select the character rendition.	*/
{
	vmode((int)*mode);								/* Select the mode.			*/
}

VIDMOVE(row, col) long *row, *col;							/* Move to a position on the screen.	*/
{
	vmove((int)(*row)-1, (int)(*col)-1 );						/* Do the move.				*/
}

VIDLINE(type, length) long *type, *length;						/* Draw a line.				*/
{
	vline((int)*type, (int)*length);						/* Do the draw.				*/
}

VIDTEXT(text, length) char *text; long *length;						/* Output text.				*/
{
	...
	if (*length > 64)
	{
		vre("VIDTEXT-E-INVALID Invalid string length [%d].  Max is 64.",(int)*length);
		return(0);
	}
...
PUSHAREA(r, c, rs, cs, s) long *r, *c, *rs, *cs; unsigned char **s;			/* Push screen area.			*/
{
...
MENUINFO(mcb,level,item,link) struct video_menu *mcb; long *level, *item, *link;	/* Return the menu level and item.	*/
{
	*level = (long)vlastlevel(mcb) + 1;						/* Get the current level.		*/
	*item = (long)vlastitem(mcb) + 1;						/* Get the current item.		*/
        *link = (long)vlastlink(mcb);							/* Get the current link.		*/
}
