@echo off
REM	Copyright (c) 1995 DevTech Migrations, All rights reserved.
REM	$Id:$
REM
REM
...
set EDE=%SRC%\EDE
REM
REM     Delete old SHIP KIT directories
REM
echo Deleting old %SHIP%
deltree /y %SHIP%
REM
...
@echo on
@echo Loading misc.
copy c:\pkware\pkunzip.exe      %SHIP%
copy %SRC%\msdos\readme.dos     %SHIP%
copy %SRC%\ede\readme.ede       %SHIP%
copy %PORT%\shipwisp.bat        %SHIP%
copy %PORT%\shipede.bat         %SHIP%
...
copy %WU%\wdiag.exe             %SHIP%\wisp\bin
...
copy %SRC%\msdos\doswisp.lis    %SHIP%\wisp\etc
copy %WU%\disprint.wcb          %SHIP%\wisp\etc
copy %WU%\disprint.mak          %SHIP%\wisp\etc
copy %WT%\words.def             %SHIP%\wisp\etc
copy %SRC%\msdos\autoexec.wsp   %SHIP%\wisp\etc
copy %SRC%\msdos\config.wsp     %SHIP%\wisp\etc
...
copy %WACU%\wruncblx.wat        %SHIP%\wisp\acu\wruncbl.wat
copy %SRC%\port\dosmake.wat     %SHIP%\wisp\acu
