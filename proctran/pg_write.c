...
void wrt_init_assign()
{
	...
	write_log("PROCTRAN",'I',"WRITE","Writing the ASSIGN items.");
	...
	else write_log("PROCTRAN","W","INVALID","Invalid ASSIGN, not a declare variable.");
	...
	if (*cur_assign->start_pos)							/* Generate a CALL STRING statement.	*/
	{
		...
		strcat(cobline,"MOVE ");
		if (*cur_assign->start_pos)						/* If have an INPUT substring.		*/
		{
	...
	else										/* else generate a STRING statement.	*/
	{
		...
		if (cfl) gen_compute(gfl,0);						/* Complete the COMPUTE statement,	*/
		else 									/* else do a STRING or MOVE.		*/
		{
			complete_assign(0,num,htyp,gfl);				/* Complete the MOVE or STRING code for	*/
		}									/*  the ASSIGN statement.		*/
	}
	if (cur_assign && cur_assign->next_item)
	{
		if (*cur_assign->next_item->type == 'S') complete_assign(1,2,htyp,gfl);
		else gen_compute(gfl,1);						/* Complete the COMPUTE statement,	*/
	}
}

void wrt_call()										/* Handles syntax for call verbs.	*/
{
	write_log("PROCTRAN",'I',"WRITE","Writing the CALL syntax.");

	strcpy(cobline,"%IDSI - Verification of ");
...
{
	write_log("PROCTRAN",'I',"WRITE","Writing the GOTO items.");
...
void wrt_if()										/* Writes syntax for if statements.	*/
{
	...
	write_log("PROCTRAN",'I',"WRITE","Writing the IF items.");
			...
			if (*cur_if->paren_value)
			{
				if (strcmp(cur_if->paren_value,")") &&  strcmp(cur_if->paren_value,"("))
			  	{							/* Put out error message if warning in if.*/
					write_log("PROCTRAN",'E',"INVALIF","Index found in IF statement. Correction needed.");
				}
...
{											/*  submit, print, commands.		*/
	...
	{										/* Write the PUTPARM call source for	*/
		if (0==strcmp(cur_prg->name,"COPY")) wrt_filecopy();			/* Is a RUN COPY command -> FILECOPY.	*/
		else if (cur_prg->func =='P') wrt_print_kw();				/* Is a PRINT command.			*/
		else if (cur_prg->func =='S') wrt_submit_kw();				/* Is a SUBMIT command.			*/
		else wrt_key_params();							/* the current command.			*/
	}

	if (cur_prg->func == 'R')							/* Is it a RUN command?			*/
	{
		if (0==strcmp(cur_prg->name,"COPY"))					/* RUN COPY?				*/
		{
			indent_line();							/* Add the correct indentation.		*/
			strcat(cobline,"PERFORM FILECOPY-SUB");
			end_prt_line(1);						/* Add end of line stuff and write line.*/

			if (*cur_prg->rtrn_cd_lbl)					/* Add setting of para label for RUN.	*/
			{
				indent_line();						/* Add the correct indentation.		*/
				strcat(cobline,"MOVE RETURN-CODE TO ");
				concat_var_prefix('R');
		...
		else	 wrt_run();							/* Write the RUN command.		*/
	}
...
static wrt_run()									/* Writes out syntax for the run command.*/
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		if (i == 0)
		{
			...
			{
				write_log("PROCTRAN",'W',"WRITE","Writing WSORT.  Check if need call to SORTINFO");
				strcpy(cur_prg->name,"WSORT");				/* Change LINK to SORT to a link to the	*/
			}								/* WSORT utility.			*/
			ctype = cur_prg->name_type[1];
		}
		else if (i == 1)                                          
	...
	indent_line();									/* Add the correct indentation.		*/
	strcat(cobline,"MOVE ");
	if (0==strcmp(cur_prg->name,"WSORT")) strcat(cobline,"\"S\"");			/* Set LINK-TYPE to S for WSORT.	*/
	else if (*cur_prg->lib == '\0' && *cur_prg->vol == '\0') strcat(cobline,"\" \"");
...
{
	write_log("PROCTRAN",'I',"WRITE","Writing the RETURN items.");
...
void wrt_rs()										/* Write out renames and scratchs.	*/
{
	...
	write_log("PROCTRAN",'I',"WRITE","Writing the RENAME/SCRATCH items.");
...
void wrt_set_extract()									/* Write out set and extract syntax.	*/
{
	...
	write_log("PROCTRAN",'I',"WRITE","Writing the SET/EXTRACT items.");
...
void wrt_readfdr()									/* Write out readfdr syntax.		*/
{
	...
	write_log("PROCTRAN",'I',"WRITE","Writing the READFDR items.");
	...
	for (i = 0; i < 3; i++)
	{
		...
		else if (*cstr == '\0') strcat(cobline,"SPACES");
		else 									/* Is an alpha numeric string.		*/ 
		{
			strcat(cobline,"\"");						/* Load the quote symbol.		*/
...
{											/* needed for all the PUTPARM calls.	*/
	...
	write_log("PROCTRAN",'I',"WRITE","Writing the PUTPARM variable items.");
	max = 0;									/* Set to no keywords in putparms.	*/
	max_len = 0;									/* Set to no max length for value.	*/
...
{											/* parameters for the CALL stmnt.	*/
	...
	write_log("PROCTRAN",'I',"WRITE","Writing the CALL PUTPARM source.");
	...
	{										/* First do all the MOVES to call vars.	*/
		...
		for (i = 8; i < 11; i++)						/* Write the test of return code stmts.	*/
		{
			indent_line();							/* Add the correct indentation.		*/
...
static wrt_call_link()									/* Write out the Call to LINK.		*/
{
	...
	if (cur_prg->number_using > 32) write_log("PROCTRAN",'E',"MAXPARMS",
...
static gen_compute(gfl,tfl)			    					/* Generate the COMPUTE statement.	*/
int gfl, tfl;
{
	indent_line();									/* Add the correct indentation.		*/
	strcat(cobline,"COMPUTE ");
...
int type;										/* PUTPARM parameters for the CALL stmnt.*/
{
	...
	char fldn[FLDLEN], len[FLDLEN];
	char *cstrp, keyw[FLDLEN];
	int cnt, i, j, br;

	write_log("PROCTRAN",'I',"WRITE","Writing the backwards reference CALL PUTPARM source.");
	...
	if (type == 1)									/* Get info. from PUTPARM structure.	*/
	{
		...
		while (cur_kwl)
		{
			if (cur_kwl->backref)						/* Test if a backwards referenced key.	*/
			{
	...
	else if (type == 2)								/* Get info. from IF structure.		*/
	{
		cnt = 0;
		for (i = 0; i < 3; i++)							/* Do for each file spec. keyword.	*/
		{
			...
			{
				cnt++;							/* Increment keyword count.		*/
				if (cnt == 1)						/* Assume is same as library.  Set if	*/
	...
	else if (type == 3 || type == 5)						/* Get info. from RENAME/SCRATCH struct */
	{										/*  or the READFDR structure.		*/
		cnt = 0;
		for (i = 0; i < 3; i++)							/* Do for each file spec. keyword.	*/
		{
			if (i == 0)
			{
				if (type == 3)
				{
					cstrp = cur_ren_sctch->name;			/* Assign the current file.		*/
					br = cur_ren_sctch->name_br;
				}
				else
				{
					cstrp = cur_rfdr->file;				/* Assign the current file.		*/
					br = cur_rfdr->name_br;
				}
			}
			else if (i == 1)
			{
				if (type == 3)
				{
					cstrp = cur_ren_sctch->lib;			/* Assign the current library.		*/
					br = cur_ren_sctch->lib_br;
				}
				else
				{
					cstrp = cur_rfdr->lib;				/* Assign the current library.		*/
					br = cur_rfdr->lib_br;
				}
			}
			else if (i == 2)
			{
				cstrp = cur_ren_sctch->vol;				/* Assign the current volume.		*/
				...
				{
					cstrp = cur_rfdr->vol;				/* Assign the current volume.		*/
					br = cur_rfdr->vol_br;
				}
			}
	...
	else if (type == 4 || type == 6)						/* Get info. from ASSIGN struct.	*/
	{										/*  or RUN USING structure.		*/
		cnt = 1;
		if (type == 6) cstrp = cur_using->var;					/* Assign the current field.		*/
		else cstrp = cur_assign->field1;
	...
	else if (type == 4 || type == 6)
	{
		...
		{
			write_log("PROCTRAN","W","MODSIZE","Possible mods. to length needed for PUTPARM.");
			strcat(cobline,"8");
		}
		strcat(cobline," TO PPARM-LEN-");
		strcat(cobline,fldn);
	...
	{
		j = 1;
		if (type == 6) cstrp = cur_using->var;					/* Assign the current file.		*/
		else cstrp = cur_assign->field1;
...
static wrt_filecopy()									/* Write out the PUTPARMs if needed and */
{											/* parameters for the CALL stmnt.	*/
	register int i;
	char cnt[FLDLEN];
	char type;

	write_log("PROCTRAN",'I',"WRITE","Writing the CALL FILECOPY source.");
	...
	{										/* First do all the MOVES to call vars.	*/
		...
		{
			if (0==strcmp(cur_pp->prname,"INPUT")) type = 'I';		/* Set which one.			*/
			else	type = 'O';
			if (cur_pp->kwcnt)						/* If have keywords.			*/
			{
				cur_pp_key = cur_pp->kwlist;
				for (i = 0; i < cur_pp->kwcnt; i++)
				{
					...
					if (0==strcmp(cur_pp_key->keywrd,"FILE")    ||
					    0==strcmp(cur_pp_key->keywrd,"LIBRARY") ||
					    0==strcmp(cur_pp_key->keywrd,"VOLUME"))
					{
						indent_line();				/* Add the correct indentation.		*/
						strcat(cobline,"MOVE ");
						...
						if (type == 'I') strcat(cobline,"IN-FILECOPY-");
						else strcat(cobline,"OUT-FILECOPY-");

						if (0== strcmp(cur_pp_key->keywrd,"FILE")) strcat(cobline,"FILE");
						else if (0== strcmp(cur_pp_key->keywrd,"LIBRARY")) strcat(cobline,"LIB");
						else if (0== strcmp(cur_pp_key->keywrd,"VOLUME")) strcat(cobline,"VOL");
						if (in_if) end_prt_line(0);		/* Add end of line stuff and write line.*/
						else end_prt_line(1);
					}
					cur_pp_key = cur_pp_key->next_item;
	...
	{										/* If a variable or return code label.	*/
		if (*cur_assign->field1 == '&' || cur_assign->type[1] == 'R')
		{
			...
			{								/* Is the TIME or DATE function.	*/
				if (htyp == 'I') strcat(cobline,"PL-N"); 		/* Is the numeric type variable.	*/
				else	strcat(cobline,"PL-A");				/* Is alpha type variable.		*/
			}
		...
		else
		{
			...
			cpos = 17;
			if (cpos + strlen(cur_assign->field1) >= 72)			/* If text goes past column 72		*/
...
static wrt_subscript(nfl,caller)							/* Process the subscript.		*/
int nfl, caller;									/* Generate a CALL STRING		*/
{
	...
	char temp[FLDLEN], ttyp[3];
	int num_var, num_val, gfl;							/* Set so get_type_len() doesn't fail.	*/

	write_log("PROCTRAN",'I',"WRITE","Writing the SUBSTRING CALL STRING items.");
	num_var = 0;
	num_val = 0;
	...
	else if (caller == 2 || caller == 3) times = nfl;
	...
	while (times)
	{
		...
		*tptr = '\0';								/* Null terminate the string.		*/
		...
		{
			strcpy(temp,"&");						/* Setup to get type of stpos var.	*/
			strcat(temp,istpos);
			get_type_len(temp,ttyp,&num_var,&num_val,'T');
			if (ttyp[1] == 'E') gfl = 1;
			...
			strcat(cobline,&temp[1]);					/* Load OFFSET into output.		*/
		}
		else
		{
			strcat(cobline,"MOVE ");
		...
		if (varcnt == 3 || varcnt == 4)
		{
			...
			get_type_len(temp,ttyp,&num_var,&num_val,'T');
			concat_var_prefix(*ttyp);					/* Concatenate the stpos var's prefix.	*/
			strcat(cobline,temp);						/* Load OFFSET into output.		*/
		}
		...
		strcat(cobline,&invar[1]);						/* Load the INPUT var.			*/
		end_prt_line(2);							/* Add end of line stuff and write line.*/
		...
		strcpy(cobline,"                          ");
		if (in_if) strcat(cobline,"     ");					/* Load the indent if in if statement.	*/
		concat_gfl_prefix(ityp);						/* Concatenate the OUTPUT var's prefix.	*/
		if (caller == 1) strcat(cobline,&cur_if->var[1]);			/* Load the OUTPUT var.			*/
