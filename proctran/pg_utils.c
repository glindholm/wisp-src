...
void write_log(facil,sever,mess,lform,p0,p1,p2,p3,p4,p5,p6,p7)				/* write a log line to the current log	*/
	...
	if (logging)
	{
		if (sever != ' ')							/* if severity set to space, skip	*/
		{
			...
			{								/* write facility, severity, message	*/
				fprintf(logfile,"%%%s-%c-%s (%d) ",facil,sever,mess,num_inlines);
			}
			else	fprintf(logfile,"%%%s-%c-%s (%d) ",facil,sever,mess,num_outlines);
		}
...
int nexttok()									/* Get position of next token from inline, and	*/
{										/*  set the next ptr as well.			*/
	...
	if (*aptr == '\0' || *aptr == LNFD || (aptr - inline >= 71)) return(FALSE); /* No more tokens left to process.		*/ 
	else return(TRUE);
}

int tststrt(cptr)								/* Test if valid start for token position.	*/
char *cptr;
{
	if (*cptr == ' '  || *cptr == ',' || *cptr == HT || *cptr == '\0' || *cptr == LNFD ||
		(cptr - inline >= 71) )						/* In the mod code/comment area.		*/
	{
		return FALSE;
	}
	else	return(TRUE);
...
static int tstkey(cptr)									/* Test if valid end for key position.	*/
char *cptr;
{
	if (*cptr == ' '  || *cptr == ',' || *cptr == HT || *cptr == '\0' || *cptr == LNFD)
	{
...
int find_colin(lptr)									/* Look for : to define proc-heading.	*/
char *lptr;
{
	while (tststrt(lptr))
	{
...
int find_keyword(keystr,keylist)							/* Find the keyword in the list.	*/
char *keystr, *keylist[];								/* Return index in keylist.		*/
{
	...
	i = 0;										/* Copy upper case token to temp string.*/
	while (tstkey(tmp_add) && *tmp_add != '=' && *tmp_add != '(') tmp_str[i++] = toupper(*tmp_add++);
	tmp_str[i] = '\0';								/* Null terminate the string.		*/
	i = 0;
	while (*keylist[i])								/* While there are keywords to compare.	*/
	{
		if (!strcmp(tmp_str,keylist[i])) return(i);				/* If a match return array pos in list.	*/
		i++;
	}
...
int *num_asn, current_row;
{
	...
	{										/* Copy test to temp var and get length.*/
		convert_to_dbl();							/* Convert ' withing string to ".	*/
		while (*aptr != '\'' && *aptr != LNFD && (aptr - inline < WPLMAX))
		{
			if (*aptr == '"')
			{
	...
	*cptr = '\0';									/* Null the string.			*/
	...
	if (in_prompt || in_message)
	{
		...
		strcpy(cur_scn_fld->scn_fld_type,"S ");					/* Load the type.			*/
...
{											/* getop = 'S' assign screen item len.	*/
	...
	write_log("PROCTRAN",'I',"GET","Getting type/size of %s from variable list.",fld);
	...
	if (!retfl)
	{
		...
		if (getop == 'A') write_log("PROCTRAN",'I',"ADDINGVAR","Adding Backwards referenced var: %s.", fieldb);
		else if (getop == 'C') write_log("PROCTRAN",'I',"ADDINGVAR","Adding subscript var: %s.", fieldb);
		else write_log("PROCTRAN",'W',"UNDEFVAR","Undefined variable: %s, need to adjust PIC clause.", fieldb);
		...
		else
		{
			...
			else if (getop == 'C');						/* If 'C' don't do anything.		*/
			else if (getop == 'L')
			{
...
char *lptr;
{
	...
	if (qcnt > 2) adj_quotes(lptr,qcnt);						/* Check if quotes are within string.	*/
}
		...
		if (*lptr == '\'')
		{
			...
			else
			{
				...
				if ( !match_sgl )					/* If have matched all the single '.	*/
				{
					...
					else write_log("PROCTRAN",'I',"NOQUOTE","No matching quote.  Check procedure code.");
				}
				else
				{							/* Check if a keyword.			*/
					if ((0==strncmp(lptr,"THEN",4)) || (0==strncmp(lptr,"AND",3)) ||
				   	    (ndx = find_keyword(lptr,proc_keywords) >= 0) )
					{
						break;
  					}
				}
				...
				if (*lptr != ',' && *lptr != '&' )
				{							/* If don't need to match double.	*/
					if (*lptr != '\'' && !match_dbl && !match_sgl)
					{
...
static convert_to_dbl()									/* Convert ' withing string to ".	*/
{
	char *laptr;
	int i, cnt;
