...
int *num_cmds, *num_var;								/* cmd then call correct format routine.*/
{
	...
	{
		write_log("PROCTRAN",'W',"VSSYSUTIL","Writing VS System Utility %s NOT SUPPORTED message.",cur_cmd->command);
		strcpy(cobline,"      *     ");
		strcat(cobline,cur_cmd->command);
		strcat(cobline," not supported.  Manual modifications needed.");
		end_prt_line(1);							/* Add end of line stuff and write line.*/
		setflag();
		return;
	}

	write_log("PROCTRAN",'I',"CHKCMD","Checking for a valid CALL command");
	...
	{
		write_log("PROCTRAN",'E',"VERIFY_CALL",
