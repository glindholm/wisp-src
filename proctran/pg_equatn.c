...
p_assign(num_asn,num_var,filler_num,current_row,num_val)				/* Process the current assign equation.	*/
int *num_asn, *num_var, *filler_num, *current_row, *num_val;
{
	...
	int setp, fvar;									/* Flag if need to set ptr back one.	*/

	astrt = NULL;									/* Set so processes corrextly.		*/
	fvar = TRUE;
	while ((tndx = find_char(*aptr,search_equations)) >= 0)				/* Loop until no more found in token.	*/
	{
		...
		switch (tndx)
		{
			case ANDSIGN:
			{
				write_log("PROCTRAN",'I',"PROCESS","Processing AND sign for ASSIGN variable in equation.");
				init_assign(num_asn);
				...
				*cstr = '\0';						/* Null the string.			*/
				...
				if (*aptr == '(')					/* If a substring with assignment.	*/
				{
					...
					else if (*cur_assign->start_pos || !fvar)	/* If already have a substring or not	*/
					{						/*  the first var in assign stmnt.	*/
						process_sub_var(*cur_assign->type,num_var,num_val,2,num_asn);
					}
				}
				fvar = FALSE;						/* Set flag not first variable.		*/
				if (*aptr == ' ') nexttok();
			...
			case OPNPRN:
			{
				...
				if (*aptr == '&' && !subfl)
				{
					write_log("PROCTRAN",'I',"PROCESS","Processing ASSIGN equation variable.");
					strcpy(cur_assign->literal,"(");		/* Set so puts ( in equation.		*/
					break;						/* Break out and process as variable.	*/
				}

				write_log("PROCTRAN",'I',"PROCESS","Processing ASSIGN substring parameters.");
				cstr = cur_assign->start_pos;
			...
			case EQUAL:
			{
				write_log("PROCTRAN",'I',"PROCESS","Processing EQUAL sign for equation.");
				...
				{
					cstr = aptr;
					while (*cstr != '.' && *cstr != ')') cstr++;	/* Check for a period char.		*/
					if (*cstr == '.')  process_back_ref(num_asn,num_var,num_val); /* Process backwards ref.	*/
				}
			...
			{
				write_log("PROCTRAN",'I',"PROCESS","Processing NUMBERS and OPERATIONS for ASSIGN.");
				if (!strcmp(cur_para->name,aptr))
				{
					write_log("PROCTRAN",'W',"INVALASGN","Invalid Assign TO Label may not be Correct.");
				}
				init_assign(num_asn);
				if (tndx == MULTIPLY || tndx == DIVIDE || tndx == MINUS || tndx == PLUS)
				{
					strcpy(cur_assign->type,"IC");			/* Is a COMPUTE statement.		*/
				}
				else	strcpy(cur_assign->type,"I ");			/* Is an integer type .			*/
			...
			case OPNQUOTE:
			{
				...
				write_log("PROCTRAN",'I',"PROCESS","Processing ASSIGN string for equation.");
				...
				{							/* Have substring field also.		*/
					if (*aptr == '\'')
					{
						...
						strcpy(cur_assign->type,"SL");		/* Assign the type as a literal string.	*/
						(*filler_num)++;
			...
			default:
			{
				write_log("PROCTRAN",'E',"INVALID","Invalid ASSIGN equation key character.");
				break;							/* No default.				*/
			}
		}
	}
...
int *num_asn;
{
	...
	{
		write_log("PROCTRAN",'I',"PROCESS","Processing LABEL for ASSIGN variable in equation.");
		init_assign(num_asn);
		cstr = cur_assign->field1;						/* Get the assign variable.		*/
		while(tststrt(aptr))
		{
		...
		*cstr = '\0';								/* Null the string.			*/
		strcpy(cur_assign->type,"IR");						/* Is an integer return code var.	*/
		next_ptr = aptr;
		if (*aptr == ' ') nexttok();
	}
}
...
int *num_var, *num_val;
{
	...
	write_log("PROCTRAN",'E',"ERRTYPE",
		"ASSIGN integer (%s) to string (%s). Manual changes required.",cur_assign->field1,prev->field1);
}
...
static int test_assign_flds()								/* Assign if should create literal.	*/
{
	...
	prev = cur_assign->prev_item;
	if (!prev && *cur_assign->start_pos) return(TRUE);				/* Have sustring and no other fields.	*/
...
int *num_asn, *num_var, *num_val;
{
	...
	aptr++;										/* Step past the (.			*/
	write_log("PROCTRAN",'I',"PROCESS","Processing Backwards reference for ASSIGN variable in equation.");
	init_assign(num_asn);
