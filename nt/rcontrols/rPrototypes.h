/*
** Copyright (c) 1994-2003, NeoMedia Technologies, Inc. All Rights Reserved.
**
** WISP - Wang Interchange Source Processor
**
** $Id:$
**
** NOTICE:
** Confidential, unpublished property of NeoMedia Technologies, Inc.
** Use and distribution limited solely to authorized personnel.
** 
** The use, disclosure, reproduction, modification, transfer, or
** transmittal of this work for any purpose in any form or by
** any means without the written permission of NeoMedia 
** Technologies, Inc. is strictly prohibited.
** 
** CVS
** $Source:$
** $Author: gsl $
** $Date:$
** $Revision:$
*/

//	Function Prototypes
BOOL CALLBACK SelectDirDlgProc ( HWND hDlg, UINT msg, WPARAM wp, LPARAM lp );
BOOL CALLBACK SelectFilesDlgProc ( HWND hDlg, UINT msg, WPARAM wp, LPARAM lp );
VOID CALLBACK ClockTimerProc ( HWND hWnd, UINT msg, UINT idEvent, DWORD dwTime );
BOOL CALLBACK SelectServerDlgProc ( HWND hDlg, UINT msg, WPARAM wp, LPARAM lp );
BOOL CALLBACK SelectSingleDirDlgProc ( HWND hDlg, UINT msg, WPARAM wp, LPARAM lp );

/*
**	History:
**	$Log: rPrototypes.h,v $
**	Revision 1.4  2003/06/18 16:43:07  gsl
**	Add CVS header and history
**	
**
*/
