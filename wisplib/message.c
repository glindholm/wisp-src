...
extern char wsx_buf;									/* Buffer for AID char.			*/
extern char msg_pfkey();
...
static $DESCRIPTOR(mbxlnm_table, "LNM$WISP_TABLE");

static $DESCRIPTOR(lognam_prefix, "WISPMBX_"); /* prefix for MBX logical names */
...
va_dcl
{
    ...
    if (strncmp(function_type, "CR", 2)==SUCCESS) {
...

	/* #ifdef VMS	*/
    else if (strncmp(function_type, "DE", 2)==SUCCESS) {
        int *return_code;
    ...
    else if (strncmp(function_type, "X", 1)==SUCCESS) {
        char *msg;
...

	/* #ifdef VMS	*/
    else if (strncmp(function_type, "CH", 2)==SUCCESS) {
        char *check_type;
...
int CreateMessagePort( char *port_name, unsigned int buf_size, char keep_flag)
{
    ...
    status = sys$crembx( prmflg, &chan, maxmsg, bufquo, promsk, acmode, 
                        &lognam_desc);
    if (status != SS$_NORMAL) {
        MESSAGE_ss_status = status;
...
check_k:
	...
	if (wsx_buf != 0x20 && wsx_buf != 0x21 && wsx_buf != 0)				/* Was there already a char in wsx_buf?	*/
	{
	...
	{
		if (inchar == 27)							/* Was an escape sequence		*/
		{
			vpushc(inchar);							/* Push the char.			*/
			inchar = vgetm();						/* Get its meta value.			*/
			wsx_buf = msg_pfkey(inchar);					/* Store it for wsxio.			*/
		}
		else
		{
...
wait_k:
	...
	if (!(sys$readef(vmbx_ef, &ef_cluster) == SS$_WASCLR))				/* Some key was pressed.		*/
	{
		...
		if (inchar == 27 && (wsx_buf == 0x20 || wsx_buf == 0))			/* Was an escape sequence		*/
		{									/* No previous key, and keyboard unlock.*/
			vpushc(inchar);							/* Push the char.			*/
			inchar = vgetm();						/* So get it.				*/
			wsx_buf = msg_pfkey(inchar);
		}
		else
		{
...
wait_b:
        ...
        if (!(sys$readef(vmbx_ef, &ef_cluster) == SS$_WASCLR))				/* Was a key pressed?			*/
	{
		...
		if (inchar == 27 && (wsx_buf == 0x20 || wsx_buf == 0))			/* Was an escape sequence		*/
		{									/* No previous key, and keyboard unlock.*/
			vpushc(inchar);							/* Push the char, and get pfkey value.	*/
			inchar = vgetm();						/* So get it.				*/
			wsx_buf = msg_pfkey(inchar);
		}
		else
	...
	if ((qio_status == SS$_NORMAL) && (iosb.status == SS$_NORMAL))
	{
		...
		free(temp);
		return(SUCCESS);							/* bytes read				*/
	}

        free(temp);
	if (return_status != SUCCESS) return(return_status);
...
int LookUpPort( char *port_name, struct mbx_list *(*node) )
{
    ...
    $DESCRIPTOR (temp_desc, templog);
    char tempequiv[2]; /* don't really need to get the translation in full */
    $DESCRIPTOR (equiv_desc, tempequiv);
    unsigned status;
    unsigned long lnmatt = LNM$M_CASE_BLIND;
...
int RemovePort( struct mbx_list *node)
{
    ...
    while (ptr != NULL) {
        if (ptr == node) { /* we found it! */
            if (prev != NULL) { /* if we're not at the head of the list */
                prev->next = ptr->next; 
            }
            else {  /* delete entry at head */
...
char msg_pfkey(inchar) int inchar;
{
	int x;
	x = vfnkey(inchar);								/* Get the value of the fn key.		*/
	return(x + ((x > 16) ? 'P' : '@'));						/* Return the correct AID byte value.	*/
}
...
va_dcl
{
	...
	long wait, func, ourret, *retcode;
	char *receiver;
	long check_t, time_int, mess_len;
	long *time_int_p, *mess_len_p;
	char *mtext;
	...
	switch (func)
	{
		case M_CREATE:	while (arg_count > 1) 
				...
				retcode	= va_arg(arg_list,long *);			/* get retcode pointer			*/
				ourret	= creport(prname);				/* dispatch 				*/
				break;
		case M_DELETE:	retcode	= va_arg(arg_list,long *);			/* get retcode pointer			*/
		...
		case M_XMIT:	mtext	= va_arg(arg_list,char *); 			/* get text pointer			*/
				mess_len_p= va_arg(arg_list,long *);			/* and length				*/
				GETBIN(&mess_len,mess_len_p,sizeof(long));
				wswap(&mess_len);
				retcode	= va_arg(arg_list,long *);			/* and retcode				*/
		...
		case M_CHECK:	check_t    = *va_arg(arg_list,char*);			/* get check type			*/
				time_int_p = va_arg(arg_list,long*);			/* and timeout value			*/
				GETBIN(&time_int,time_int_p,sizeof(long));		/* copy to our int 			*/
				wswap(&time_int);					/* swap if necessary			*/
				receiver   = va_arg(arg_list,char*);			/* pointer to receiving area		*/
				mess_len_p = va_arg(arg_list,long*); 			/* pointer to message len		*/
				arg_count -= 4;
				if (arg_count == 2) chptr = va_arg(arg_list,char*);	/* if two args left ignore first	*/
				retcode    = va_arg(arg_list,long *);
				ourret     = getmsg(prname,check_t,time_int,mess_len_p,receiver);	/* get the message	*/
				break;
		case M_INV:	ourret = FAILURE;
				break;
		default:	break;
	}
	PUTBIN(retcode,&ourret,sizeof(long));						/* copy our ret code to their area*/
...
long	wait;
{
	...
	msg.mtype = MSGTYPE;								/* assign arbitrary type		*/

	if (msgsnd(msqid,&msg,mlen,wait?0:IPC_NOWAIT)<0)				/* call msg send routine		*/
	{
...
static getmsg(prname,check_t,time_int,mess_len,receiver)
...
long check_t,time_int,*mess_len;
{
	...
	wait_time = wait_key = timeout = gotmsg = 0;
	GETBIN(&mlen,mess_len,sizeof(long));
	port_key = keyval(prname,0);
	if (port_key == (key_t) -1)
	{
		return CH_NO_PORT;
	...
	if (status < mlen)
	{
		...
		PUTBIN(mess_len,&mlen,sizeof(long));					/* pass length back			*/
		wswap(mess_len);
	}
...
int create;
{
	...
	if (!create && access(keypath,0)) return (key_t) -1;
	
	keyfile=fopen(keypath,"w");
	if( !keyfile )
	{
		werrlog(ERRORCODE(16),"create",keypath,0,0,0,0,0,0,0);
		return( -1 );
	}
	fprintf(keyfile,"\n");
	fclose(keyfile);
