...
#define		ROUTINE		19000
	...
	long	*starter,*counter,*file_count;
	long	l_starter,l_counter,l_file_count;
	...
	arg_count--;
	starter = va_arg(the_args, long*);						/* Get address of the start count.	*/
	arg_count--;
	counter = va_arg(the_args, long*);						/* Get address of the number to list.	*/
	arg_count--;
	...
	{
		file_count = va_arg(the_args, long*);					/* Get address of the file count.	*/
		arg_count--;
		l_file_count = 0;
	}
	receiver_type = NULL;
...
$DESCRIPTOR(l_desc,l_vol);								/* Make a descriptor of the volume.	*/
$DESCRIPTOR(t_desc,template);
$DESCRIPTOR(r_desc,result);
$DESCRIPTOR(lnm_file,"LNM$FILE_DEV");
	...
	{
		if (l_vol[0] != ' ')							/* If there is a volume provided...	*/
		{
	...
	pos = 0;									/* Position in list of files found.	*/
	cnt = 0;									/* Count of files loaded into receiver.	*/
                                                        

	status = RMS$_NORMAL;

	while ((status == RMS$_NORMAL || status == RMS$_PRV) && ( cnt < *l_counter_p))
	{
		...
		{                                                                                                                 
			...
			{ 
				...
				if ( status2 == SS$_NORMAL )				/* if all went well.			*/
				{
					if ( ! lib_search )
					{

						for( i=0, j=0; 				/* Load library into buf [dir]		*/
						     j<8 && i<items[0].clen && 
						     items[0].ptr[i] != ']' && items[0].ptr[i] != '>'; i++ )
						{
							if ( items[0].ptr[i] != '[' && items[0].ptr[i] != '<' )
								l_buf[j++] = items[0].ptr[i];
						}
						l_buf += 8;				/* point to the filename.		*/

	
						for( i=0, j=0; j<8 && i<items[1].clen; i++ ) /* Load file into buf		*/
						{
							l_buf[j++] = items[1].ptr[i];
						}
						l_buf += 8;				/* point to next receiver		*/
					...
					else /* LIBRARY FIND */
					{
						...
						for( i=0, j=0; j<8 && i<items[1].clen; i++ ) /* load library into buf		*/
						{
							l_buf[j++] = items[1].ptr[i];
						}
						l_buf += 16;				/* point to next receiver		*/

					}
				}
				else
				{
					werrlog(ERRORCODE(8),result,status,0,0,0,0,0,0);
					l_buf += 16;
				}
			}
		}
	...
	*l_counter_p = cnt;								/* Set the return count.		*/
	...
	if ( *l_file_count_p == 0 )
	{
		while ( status == RMS$_NORMAL || status == RMS$_PRV )			/* If the pointer's set	& there're more	*/
		{                                                                       /* return the total count.		*/
			t_desc.dsc$w_length = strlen(template);				/* Set the length in descriptor.	*/
			status = LIB$FIND_FILE(&t_desc,&r_desc,&context,0,0,0,0);	/* look for a file			*/
			if (status == RMS$_NORMAL) pos++;				/* Found, add 1 to the counter.		*/
		}
		*l_file_count_p = pos;							/* If the pointer is set, return value.	*/
	}
...
static int found_count;									/* Total matches found.			*/
static int stop_count;									/* 0 = Count all.			*/
static int returned_count;								/* Number matches returned.		*/
static int requested_count;								/* Number matches requested.		*/
static int item_start;									/* First match to return		*/
static int item_count;									/* Current match position		*/
...
static int build_list(vspec,lspec,fspec)						/* the VOLUMES.				*/
char *vspec,*lspec,*fspec;
{
	logical_id *p;
	extern 	logical_id *logical_list;						/* List of VOLUMES form LGMAP		*/
	char	*vol, *vol_path;

	if ( haswc(vspec) )
	{
											/* MATCH THE VOLUME			*/
		for (p=logical_list; p; p=(logical_id *)p->next)			/* for loop to find matching volume 	*/
		{
			vol      = p->logical;
	...
	else										/* NO WILDCARDS IN VOLUME		*/
	{
		...
		if ( access( vol_buf, 00 ) == 0 )					/* See if it exists			*/
		{
...
static int build_lib_list(vol,vol_path,lspec,fspec)
char *vol, *vol_path;
char *lspec, *fspec;
{
	...
	else if (! haswc(lspec) )							/* NO WILDCARDS IN LIBRARY		*/
	{
		...
		sprintf(tmpbuf,"%s/%s",vol_path,lib_buf);

		if ( access( tmpbuf, 00 ) == 0 )					/* See if it exists			*/
		{
	...
	else while ((libname = nextfile(vol_path,&vol_context)) != NULL)
	{
		       ...
		       wcmatch(lspec,libname,FALSE,'*','?') )               )
		{
			...
			sprintf(tmpbuf,"%s/%s",vol_path,libname);			/* Make path to call stat		*/
			if ( stat(tmpbuf,&statbuf)==0 )					/* make sure we can access it 		*/
			{
				 if (statbuf.st_mode & S_IFDIR)				/* is it a dir? 			*/
				{
...
static int build_file_list(vol,vol_path,lib,fspec)
char *vol, *vol_path;
char *lib;
char *fspec;
{
	...
	sprintf(dirpath,"%s/%s",vol_path,lib);
	while ((filename = nextfile(dirpath,&lib_context)) != NULL)
	{
		...
		if ( wcmatch(fspec,cmp_file,FALSE,'*','?') )
		{
			...
			sprintf(tmpbuf,"%s/%s",dirpath,filename);			/* Make path to call stat		*/
			if ( stat(tmpbuf,&statbuf)==0 )				    	/* make sure we can access it 		*/
			{
...
static int pass_back(vol,lib,fil)							/* Load the results into reciever	*/
char *vol, *lib, *fil;
{
...
static	int	search_mode;
static	long	found_count;								/* Total matches found.			*/
static	long	stop_count;								/* 0 = Count all.			*/
static	long	returned_count;								/* Number matches returned.		*/
static	long	requested_count;							/* Number matches requested.		*/
static	long	item_start;								/* First match to return		*/
static	int	item_count;								/* Current match position		*/
...
char *lspec, *fspec;
{
	...
	else if (! haswc(lspec) )							/* NO WILDCARDS IN LIBRARY		*/
	{
		...
		sprintf(tmpbuf,"%s\\%s",vol_path,lib_buf);				/* MSDOS path with back slash		*/
	...
	else while ((libname = nextfile(vol_path,&vol_context)) != NULL)
	{
		       ...
		       wcmatch(lspec,libname,FALSE,'*','?') )               )
		{
			...
			sprintf(tmpbuf,"%s\\%s",vol_path,libname);			/* Make MSDOS path to call stat		*/
			if ( stat(tmpbuf,&statbuf)==0 )					/* make sure we can access it 		*/
			{
				 if (statbuf.st_mode & S_IFDIR)				/* is it a dir? 			*/
				{
...
char *fspec;
{
	...
	sprintf(dirpath,"%s\\%s",vol_path,lib);						/* MSDOS path; with back-slash.		*/
	while ((filename = nextfile(dirpath,&lib_context)) != NULL)
	{
		...
		if ( wcmatch(fspec,cmp_file,FALSE,'*','?') )
		{
			...
			sprintf(tmpbuf,"%s\\%s",dirpath,filename);			/* Make MSDOS path to call stat		*/
			if ( stat(tmpbuf,&statbuf)==0 )				    	/* make sure we can access it 		*/
			{
				 if (!(statbuf.st_mode & S_IFDIR))			/* make sure it's not a dir 		*/
				{
...
long	*start;										/* Start table at this ordinal file.	*/
long	*max;										/* Maximum and actual count of names.	*/
char	*table;										/* Table to hold found file names.	*/
long	*size;										/* Size in bytes of each table entry.	*/
long	*total;										/* Total number of wildcard matches.	*/
{
