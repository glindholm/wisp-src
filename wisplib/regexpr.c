...
$Header: /disk1/neomedia/RCS/wisp/lib/regexpr.c,v 1.10 2000-03-13 14:12:32-05 gsl V4_4_01 $
...
char *dummy;
{
	...
	regcmpb = (struct re_pattern_buffer *)calloc(1,sizeof(struct re_pattern_buffer));
	regcmpfastmap = (char*) calloc(256,sizeof(char));
	
	regcmpb->allocated=0;
	regcmpb->buffer = 0;
	regcmpb->translate = 0;
	regcmpb->fastmap = regcmpfastmap;
	
	dummy = re_compile_pattern(pat, strlen(pat), regcmpb);
	
	if (!dummy)
	{
		re_compile_fastmap(regcmpb);
		return (char*)regcmpb;
	}
	return 0;
}
...
char *buf;
{
	...
	st=re_search((regexp_t)pat, buf, strlen(buf), 0, strlen(buf), &regs);
	    
	if (st == -1) return NULL;
	else return buf+regs.end[0];
}
